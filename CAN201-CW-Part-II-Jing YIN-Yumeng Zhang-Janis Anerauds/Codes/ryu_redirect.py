        ...
        if out_port != ofproto.OFPP_FLOOD:
            if eth.ethertype == ether_types.ETH_TYPE_IP:
                ...
                #  if TCP Protocol
                    ...
                    if srcip == "10.0.1.5" and dstip == "10.0.1.2":
                    	#to server1 -> server2
                        match1 = parser.OFPMatch(eth_type=0x0800, in_port=1, ipv4_src=srcip, ipv4_dst=dstip)
                        actions1 = [
                            parser.OFPActionSetField(eth_dst="00:00:00:00:00:02"),
                                    parser.OFPActionSetField(ipv4_dst="10.0.1.3"),
                                    parser.OFPActionOutput(3)]
                        self.add_flow1(datapath, 1, match1, actions1)            	

                        #to client
                        match2 = parser.OFPMatch(eth_type=0x0800, in_port=3, ipv4_src="10.0.1.3", ipv4_dst="10.0.1.5")
                        actions2 = [
                            parser.OFPActionSetField(eth_src="00:00:00:00:00:01"),
                                    parser.OFPActionSetField(ipv4_src="10.0.1.2"),
                                    parser.OFPActionOutput(1)]
                        
                        self.add_flow1(datapath, 1, match2, actions2)
                        return
